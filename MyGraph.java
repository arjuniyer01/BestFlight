...
public class MyGraph<T> implements GraphADT<T> {
	...
	protected class Path implements Comparable<Path> {
		...
		public Path(Path source) {
			...
			this.dataSequence = new LinkedList<T>(source.dataSequence);
			this.end = source.end;
		}
	...
	protected Path dijkstrasShortestPath(T start, T end) throws NoSuchElementException {
		...
		// Checks if passed data exists in the graph as vertices and throws a
		// NoSuchElementException if not
		if (!this.containsVertex(start))
			throw new NoSuchElementException("Vertex with data " + start + " does not exist in graph.");
		else if (!this.containsVertex(end))
			throw new NoSuchElementException("Vertex with data " + end + " does not exist in graph.");

		// Instantiating empty PriorityQueues to avoid passing null references
		PriorityQueue<MyGraph<T>.Path> newPossiblePaths = new PriorityQueue<MyGraph<T>.Path>();
		PriorityQueue<MyGraph<T>.Path> availablePaths = new PriorityQueue<MyGraph<T>.Path>();
		Path finalPath = null;

		// Adding initial vertex to the allPaths PriorityQueue
		availablePaths.offer(new Path(vertices.get(start)));

		// executes the following while the finalPath is null
		while (finalPath == null) {
			
			// Polls availablePaths PriorityQueue to get the shortest path and passes it to
			// a helper method that returns newly available paths and saves it in
			// newPossiblePaths PriorityQueue
			newPossiblePaths = updatePath(availablePaths.poll());

			// Parses through all the newly available paths and offers them to
			// availablePaths
			for (Path path : newPossiblePaths)
				availablePaths.offer(path);

			// checks if the polled path out of availablePaths ends at the desired end node
			// and if yes, assigns this path to the finalPath
			if(!availablePaths.isEmpty())
				if (availablePaths.peek().end.data.equals(end))
					finalPath = availablePaths.poll();

			// If there are no more paths available to traverse and finalPath has also not
			// been initialized, that means a path between the start and end vertex does not
			// exist and hence we throw a NoSuchElementException
			if (availablePaths.isEmpty() && finalPath == null && newPossiblePaths.isEmpty())
				throw new NoSuchElementException("A path between " + start + " and " + end + " does not exist.");

		}
		// Prints optimal path between the points and its cost selected by Djikstra's
		// algorithm

		return finalPath; // returns the finalPath
	}

	/**
	 * Takes the polled path from availablePaths as a parameter and returns a
	 * PriorityQueue with all possible paths leading out from this polled path
	 * 
	 * @param poll - Polled path from availablePaths
	 * @return newPossiblePaths - PriorityQueue of new possible paths from the
	 *         polled path
	 */
	private PriorityQueue<MyGraph<T>.Path> updatePath(MyGraph<T>.Path poll) {

		// Initiates empty PriorityQueue newPossiblePaths to avoid passing null
		// references
		PriorityQueue<MyGraph<T>.Path> newPossiblePaths = new PriorityQueue<MyGraph<T>.Path>();
		// Path instance to store and work on a copy of polled path
		MyGraph<T>.Path newPath;

		boolean br = false;

		// Case: There are no edges leading away from the Vertex
		if (poll.end.edgesLeaving.size() == 0) {
			return newPossiblePaths; // empty PriorityQueue returned as there are no new paths
		}

		// Iterates through all the edges leaving the vertex
		for (Edge edge : poll.end.edgesLeaving) {

			br = false;
			newPath = new Path(poll); // creates a copy of polled path that is unique to a certain edge

			// Iterates through polled path to make sure it does not return to a
			// previously visited vertex
			for (T data : poll.dataSequence) {
				if (data.equals(edge.target.data))
					br = true;
			}

			// continues to next edge if target vertex is already in the polled path
			if (br)
				continue;

			// extends and forms a new path otherwise
			newPath.extend(edge);
			newPossiblePaths.offer(newPath); // adds this new path to newPossiblePaths PriorityQueue

		}

		return newPossiblePaths; // returns all new paths as a PriorityQueue
	}
